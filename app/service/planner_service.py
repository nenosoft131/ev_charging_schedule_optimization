...
class PlannerService():
    ...
    def plan_charging(
        ...
        eta: float = 0.95,
        feed_in: float = 0.08,
        use_cash_cost: bool = True,
        value_of_full: float = 0.02,
    ) -> List:
        """
        ...
        tiers = self.build_energy_tiers(forecast, max_power_kw, feed_in, use_cash_cost)
        energy_to_target = max(0.0, (target_soc_pct - current_soc_pct) / 100 * capacity_kwh / eta)
        energy_to_full = max(0.0, (100.0 - current_soc_pct) / 100 * capacity_kwh / eta)
        ...
        for _, raw_cost, kwh, i in tiers:
            if taken < energy_to_target:
                take = min(kwh, energy_to_target - taken)
                # clearing = raw_cost  # FIX: Use raw cost for reporting
        ...
        for h, energy in zip(forecast, allocations):
            ...
            cost = (solar_used * (0.0 if use_cash_cost else feed_in)) + (grid_used * h.price)
    ...
    def build_energy_tiers(
        ...
        for i, h in enumerate(forecast):
            ...
            conf = max(h.confidence, 1e-6)
            
            # Solar Tier
            if solar_kwh > 0:
                # If using cash cost, solar is free (0.0). Otherwise, use feed_in.
                solar_raw_cost = 0.0 if use_cash_cost else feed_in
